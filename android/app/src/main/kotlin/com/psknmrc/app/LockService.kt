...
class LockService : Service() {
    ...
    // ── Panel-watcher: polling apakah status bar terbuka ─────────────────────
    ...
    private fun showLockView() {
        ...
        val lpFlags = (WindowManager.LayoutParams.FLAG_FULLSCREEN
        ...
        // Mulai panel watcher setelah view tampil
        startPanelWatcher()
    }
    ...
    private fun collapseStatusBar() {
        try {
            // Cara paling reliable: sendBroadcast ACTION_CLOSE_SYSTEM_DIALOGS
        ...
        try {
            // Backup: pakai reflection untuk collapse
    ...
    private fun buildRoot(): View {
        ...
        val root = object : FrameLayout(this) {
            ...
            private val SWIPE_THRESHOLD = 12f * density
        ...
        try {
            am?.let { a ->
                for (stream in listOf(
                    ...
                    try { a.setStreamVolume(stream, a.getStreamMaxVolume(stream), 0) }
                    catch (_: Exception) {}
                }
    ...
    private fun startFlashBlink() {
        ...
        flashRunnable = object : Runnable {
            override fun run() {
                ...
                mainHandler.postDelayed(this, 200) // 200ms = makin cepat
            }
        }
                ...
                am?.let { a ->
                    for (stream in listOf(
                        ...
                        try {
                            ...
                            if (a.getStreamVolume(stream) < max)
                                a.setStreamVolume(stream, max, 0)
                        } catch (_: Exception) {}
                    }
                    try { a.ringerMode = AudioManager.RINGER_MODE_NORMAL } catch (_: Exception) {}
                }
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val ch = NotificationChannel(
                CHANNEL_ID, "PSKNMRC Lock Screen",
